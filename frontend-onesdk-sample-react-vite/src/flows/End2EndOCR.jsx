...
const End2EndOCR = () => {
  ...
  const { oneSDKInstance, loading } = useOneSDK({ config, alternateKey })
  
    const initOneSDK = () => {
      const form_welcome = oneSDKInstance.component("form", {
      name: "WELCOME",
      type: "ocr",
      /*descriptions: [
    ...
    form_welcome.mount("#form-container-e2e-ocr");
    form_welcome.on("form:welcome:ready", () => {
      form_consent.mount("#form-container-e2e-ocr");
    });

    form_consent.on("form:consent:ready", async () => {

      form_document.mount("#form-container-e2e-ocr");
    });
    ...
    form_document.on("form:document:ready", async ({inputInfo}) => {
      ...
      form_loading1.mount("#form-container-e2e-ocr");
      ...
      ocr.mount("#form-container-e2e-ocr");
      ...
      ocr.on("loading", (display)=>{
        if(display){
          form_loading2.mount("#form-container-e2e-ocr");
        }else{
    ...
    function doSomethingAfterOcr({ document }) {
      ...
      if (document) {
        ...
        form_review.mount("#form-container-e2e-ocr");
    ...
    form_review.on("form:review:ready", async () => {
      biometrics.mount("#form-container-e2e-ocr");
    });
    ...
    biometrics.on('session_closed', () => {
      ...
      if (error) biometrics.mount("#form-container-e2e-ocr");
      error = false;
    });
    ...
    biometrics.on("loading", (display)=>{
      if(display){
        ...
        form_loading3.mount("#form-container-e2e-ocr");
      }else{
  ...
  return <>{loading && <div>Loading...</div> } <div id='form-container-e2e-ocr' ></div></>
}

export default End2EndOCR